...
FILE_NAME=phosh-antispam-${VERSION}-${ARCH}.msi
mkdir -p "$BUILD_DIR"
mkdir -p "$INSTALL_DIR"


TEMP_DIR=$(mktemp phosh-antispam.XXXXX -d -p /tmp)
...
if [ "$OPTIMIZE" ]; then
  ...
  sed '/adwaita/d' phosh-antispam.wxs > temp.wxs
  WXS_FILE="temp"
fi
...
do
 ...
 Name=\"Anti-Spam\" WorkingDirectory=\"INSTALLDIR\" $icon\
